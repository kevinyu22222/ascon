...
//////////////////////////////////////////////////////////////////////////////////
// ascon_core_aead.sv
// Ascon-AEAD128 encryption and decryption only.
// Hash (HASH256), XOF (XOF128), and CXOF (CXOF128) modes have been removed.
//////////////////////////////////////////////////////////////////////////////////
...
module ascon_core(
  ...
  // ---------------------------------------------------------------------------
  // FSM states  (SQZ_HASH removed — only AEAD paths remain)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Register signals  (hash_cnt removed)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Registers  (reg_cnt_i shrunk: hash_cnt removed, 8 bits instead of 10)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Event signals  (mode_hash_xof removed; sqz_hash* removed)
  // ---------------------------------------------------------------------------
  logic last_abs_blk;
  ...
  assign abs_msg      = abs_msg_part && (bdo_valid && bdo_ready);   // simplified: mode_enc_dec always true
  ...
  // sqz_hash* signals removed entirely
  ...
  // AEAD blocks are always W128 wide — W64 (hash-rate) conditions removed
  assign last_abs_blk =
    (abs_ad  && (word_cnt_q == (W128 - 1))) ||
    (abs_msg && (word_cnt_q == (W128 - 1)));

  assign add_ad_pad  = (fsm_q == PAD_AD)  || (abs_ad  && (bdi_valid != '1));
  // dom_sep_done && eoi_q condition removed (was CXOF-specific)
  ...
  always_comb begin
    ...
    unique case (fsm_q)
      ...
      ABS_MSG: begin
        ...
        bdo_type  = D_MSG;
        bdo_eot   = bdi_eot;
        // mode_hash_xof bdo suppression removed
      end
  ...
  always_comb begin
    ...
    // Initialization
    if (idle_done) begin
      // Only AEAD modes: hash/XOF/CXOF branches removed
      fsm_d = key_valid ? LD_KEY : LD_NPUB;
    end
    if (ld_key_done)  fsm_d = LD_NPUB;
    if (ld_npub_done) fsm_d = INIT;
    if (init_done)    fsm_d = KADD_2;  // hash/XOF/CXOF branches removed
    ...
    if (abs_ad_done) begin
      if (bdi_valid != '1) begin
        ...
        // W64 (hash-rate) condition removed
      end
    end
    if (fsm_q == PAD_AD) fsm_d = PRO_AD;

    if (pro_ad_done) begin
      if      (ad_eot_q == 0) fsm_d = ABS_AD;  // more AD blocks to absorb
      else if (ad_pad_q == 0) fsm_d = PAD_AD;  // need an all-pad block
      else                    fsm_d = DOM_SEP;  // AEAD only — CXOF branch removed
    end
    ...
    if (abs_msg_done) begin
      if (bdi_valid != '1) begin
        fsm_d = KADD_3;                              // partial → finalize
        // mode_hash_xof branch (→ FINAL) removed
      end else begin
        if (word_cnt_q != (W128 - 1)) fsm_d = PAD_MSG;  // full word, block not done
        else                          fsm_d = PRO_MSG;   // full block → permute
        // W64 hash-rate condition removed
      end
    end
    ...
    if (pro_msg_done) begin
      ...
      // else: stay put (counter still running)
    end
  ...
  always_comb begin
    ...
    // Hash/XOF/CXOF state initialisation removed
  ...
  // hash_cnt always_comb block removed entirely

  always_comb begin
    round_cnt_d = round_cnt_q;
    unique case (fsm_d)
      INIT:    round_cnt_d = ROUNDS_A;
      PRO_AD:  round_cnt_d = ROUNDS_B;   // CXOF ROUNDS_A branch removed
      PRO_MSG: round_cnt_d = ROUNDS_B;   // hash ROUNDS_A branch removed
